...
This framework provides a practical guide for choosing the right evaluation type. For example, in Malaysia, **CUA** is often preferred for **HTA** assessments due to its comprehensive approach, while **CBA** might be used for cross-sectoral policy decisions, such as comparing health and education investments.
...
1. **Calculating ICER for a New Drug**  
    ...
    - Calculate the **Incremental Cost-Effectiveness Ratio (ICER)** using the formula: $\text{ICER} = \frac{\Delta \text{Cost}}{\Delta \text{Effectiveness}}$
